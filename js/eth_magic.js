...
const contract_address = "0x14Bb39A7d0C89CA5De24E959F7C782690c126720";
...
function contract_init() {
  if (typeof web3.eth.accounts[0] != "undefined") {
    ...
    (async () => {
      await web3.eth.getBlockNumber(function (err, ress) {
...
function start_game_contract() {
  if (typeof web3.eth.accounts[0] != "undefined") {
    ...
    rig_wars_contract.StartNewMiner.sendTransaction(
      { from: account, gasPrice: game.default_gas_price },
      function (err, ress) {
...
function GetCurrentICOCycle() {
  rig_wars_contract.GetCurrentICOCycle.call(
    { from: account },
    function (err, ress) {
      if (!err) {
...
function GetICOData(ico_id) {
  rig_wars_contract.GetICOData.call(
    ico_id,
    { from: account },
    function (err, ress) {
      if (!err) {
...
function GetMinerICOData(ico_id) {
  rig_wars_contract.GetMinerICOData.call(
    account,
    ico_id,
    { from: account },
    function (err, ress) {
      if (!err) {
        if (typeof ress[0] != "undefined") {
          ...
          console.log(
            game.ico_personal_fund,
            game.ico_personal_share,
            game.ico_personal_lastclaim
          );
        }
      } else {
...
function GetMinerUnclaimedICOShare() {
  rig_wars_contract.GetMinerUnclaimedICOShare.call(
    account,
    { from: account },
    function (err, ress) {
      if (!err) {
...
function buy_rig(rigID, count) {
  ...
  rig_wars_contract.UpgradeRig.sendTransaction(
    rigID,
    count,
    { from: account, gasPrice: game.default_gas_price },
    function (err, ress) {
...
function buy_rig_eth(rigID, count) {
  ...
  rig_wars_contract.UpgradeRigETH.sendTransaction(
    rigID,
    count,
    { from: account, value: wei_price, gasPrice: game.default_gas_price },
    function (err, ress) {
      waitForReceipt(ress, function (receipt) {
        console.log("Force!");
        update_balance(1);
...
function buy_army(id, count, eth) {
  ...
  if (eth > 0) {
    // ETH-s shoping
    rig_wars_contract.BuyTroop.sendTransaction(
      id,
      count,
      {
        from: account,
...
function ReleaseICO() {
  ...
  rig_wars_contract.ReleaseICO.sendTransaction(
    { from: account, gasPrice: game.default_gas_price },
    function (err, ress) {
      waitForReceipt(ress, function (receipt) {
        console.log("Force!");
...
function FundICO(amount) {
  ...
  rig_wars_contract.FundICO.sendTransaction(
    amount,
    { from: account, gasPrice: game.default_gas_price },
    function (err, ress) {
      waitForReceipt(ress, function (receipt) {
...
function WithdrawICOEarnings() {
  ...
  rig_wars_contract.WithdrawICOEarnings.sendTransaction(
    { from: account, gasPrice: game.default_gas_price },
    function (err, ress) {
      waitForReceipt(ress, function (receipt) {
        console.log("Force!");
...
function buy_boost(price) {
  ...
  rig_wars_contract.BuyBooster.sendTransaction(
    { from: account, value: value, gasPrice: game.default_gas_price },
    function (err, ress) {
      waitForReceipt(ress, function (receipt) {
        console.log("Force!");
        update_balance(1);
...
function buy_upgrade(id) {
  ...
  rig_wars_contract.BuyUpgrade.sendTransaction(
    id,
    {
      from: account,
      value: web3.toWei(boost_data.price),
      gasPrice: game.default_gas_price,
    },
    function (err, ress) {
      waitForReceipt(ress, function (receipt) {
...
function save_game() {
  if (typeof web3.eth.accounts[0] != "undefined") {
    ...
    rig_wars_contract.UpdateMoney.sendTransaction(
      { from: account, gasPrice: game.default_gas_price },
      function (err, ress) {
        waitForReceipt(ress, function (receipt) {
          console.log("Force!");
...
function attack_address(address) {
  // Attack(address defenderAddr) public
  if (typeof web3.eth.accounts[0] != "undefined") {
    rig_wars_contract = web3.eth.contract(abi).at(contract_address);

    rig_wars_contract.Attack.sendTransaction(
      address,
      { from: account, gasPrice: game.default_gas_price },
      function (err, ress) {
        waitForReceipt(ress, function (receipt) {
...
function debug_devfund() {
  if (typeof web3.eth.accounts[0] != "undefined") {
    ...
    rig_wars_contract.WithdrawDevFunds.sendTransaction(
      { from: account, gasPrice: game.default_gas_price },
      callback
    );
  } // No Metamask Address Found!
...
function ClaimPersonalShare() {
  if (typeof web3.eth.accounts[0] != "undefined") {
    ...
    rig_wars_contract.WithdrawPotShare.sendTransaction(
      { from: account, gasPrice: game.default_gas_price },
      function (err, ress) {
        waitForReceipt(ress, function (receipt) {
          console.log("Force!");
...
function GetTotalMinerCount(callback) {
  ...
  rig_wars_contract.GetTotalMinerCount.call(
    { from: account },
...
function GetMinerAt(id, callback) {
  ...
  rig_wars_contract.GetMinerAt.call(
    id,
    { from: account },
    function (error, result) {
      if (!error) {
...
function GetMinerData(address, callback) {
  ...
  rig_wars_contract.GetMinerData.call(
    address,
    { from: account },
    function (error, result) {
      if (!error) {
...
function GetPVPData(address, callback) {
  ...
  rig_wars_contract.GetPVPData.call(
    address,
    { from: account },
    function (error, result) {
      if (!error) {
